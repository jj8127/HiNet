"""Partial 4-bit Quantization Aware Training script."""
import os
import numpy as np
...
def get_4bit_qconfig():
    activation = tq.default_fake_quant.with_args(quant_min=0, quant_max=15)
...
def mark_quant_layers(module):
    for child in module.children():
        ...
        if isinstance(child, nn.Conv2d):
            child.qconfig = get_4bit_qconfig()
...
def main(pretrained=None, epochs=1, calib_steps=5):
    label = f"4bit_ep{epochs}_calib{calib_steps}"
    ...
    save_path = os.path.join("model", f"model_qat4bit_{label}.pt")
